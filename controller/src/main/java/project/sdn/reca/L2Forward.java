    ...
    public PacketResult receiveDataPacket(RawPacket inPkt) {
        ...
        if (formattedPak instanceof Ethernet) {
            ...
            // Switch
                ...
                if (!numberOfMacs.isEmpty() && srcMAC_val < 100 && dstMAC_val < 100) {
                	if (numberOfMacs.containsKey(srcMAC_val)) {
                		count = numberOfMacs.get(srcMAC_val) + 1L;
                		numberOfMacs.remove(srcMAC_val);
                		numberOfMacs.put(srcMAC_val, count);
                	}
                	else {
                		numberOfMacs.put(srcMAC_val, 1L);
                	}
                }
                else if (numberOfMacs.isEmpty() && srcMAC_val < 100 && dstMAC_val < 100) {
                	numberOfMacs.put(srcMAC_val, 1L);
                }
                
                if (numberOfMacs.get(srcMAC_val) > 2) {
                	numberOfMacs = null;
                	System.out.println("Pointing switch to another controller!");
                	try {
                		Process p = Runtime.getRuntime().exec("sudo ovs-vsctl del-controller s1");
                		p = Runtime.getRuntime().exec("sudo ovs-vsctl set-controller s1 tcp:192.168.56.1:6633");
                	}
                	catch(IOException e){ System.out.println("Error executing commands!");}
                }
