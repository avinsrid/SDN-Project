                ...
                if (dst_connector != null) {
                    ...
                    if (checkIfDestInNetwork) {
                        ...
                        try {
                            ...
                            PortDiscovery portDiscover = new PortDiscovery();
                            Set<NodeConnector> setNodeConn = portDiscover.getPortSet();
                            if (!setNodeConn.isEmpty()) {
                                floodPacket(inPkt);
                            }
                            else { System.out.println("L2Forward::receiveDataPacket(): Broken Link to other network, not forwarding packet"); }
                        }
                        catch(JSONException je) {System.out.println("L2Forward::receiveDataPacket() Caught JSONException"); }
                    }                
                }
            }
        }
