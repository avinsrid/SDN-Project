/* COMSE-6998 SDN Project Fall 2014
   ...
   Region Optimization Child Controller-1
...
public class RO implements IRouting, ITopologyManager {
    ...
    private int MasterPort = 41102; // CHILD CONTROLLER 1
    private Set<NodeConnector> setinOtherController = new HashSet<NodeConnector>();
    private List<Host> HostsinOtherController;
    NodeConnector other;
    ...
    public void roImplementation () {
        ...
        NodeConnector temp = NodeConnector.fromString("OF|6@OF|00:00:00:00:00:00:00:02");
        List<Host> tempHost = topologyManager.getHostsAttachedToNodeConnector(temp);
        if(tempHost!=null)
        old = tempHost.size();
        else old =0;
        ...
        while (true) {
            if (System.currentTimeMillis() - Timer > 60000) {
                ...
                NodeConnector[] arrayofNodeconnectors = setOfNodeconnectors.toArray(new NodeConnector[setOfNodeconnectors.size()]);
                
                String NCid;
                for(NodeConnector NC : arrayofNodeconnectors)
                {
                    NCid = NC.getNodeConnectorIdAsString();
                    String k = "OF|6";
                    System.out.println(" \n NodeID for NodeConnector" + NC + " is " + NCid + NCid.length() + " " + k.length());
                    if(NCid.equals("OF|6") )
                    {
                        other = NC;
                        setinOtherController.add(NC);
                        setOfNodeconnectors.remove(NC);
                    }
                }
            
                System.out.println("\n\n Set of Nodes in Present Controller is " + setOfNodeconnectors);
                System.out.println("\n\n Set of Nodes in Other Controller is " + setinOtherController);
                if(other!=null) {
                    HostsinOtherController = topologyManager.getHostsAttachedToNodeConnector(other); }
                System.out.println("\n\n Number of Hosts in Present Controller is " + setOfNodeconnectors.size());
                if(HostsinOtherController!=null)
                {
                System.out.println("\n\n List of Hosts in Other Controller is " + HostsinOtherController);
                  System.out.println("\n\n Number of Hosts in Other Controller is " + HostsinOtherController.size()); }
                ...
                {
                    System.out.println("\n\nRO::roImplementation(): nodeswithNodeConnectorHosts is a Not Empty Set");
                    Set<Node> setOfNodeswithHosts = nodeswithNodeConnectorHosts.keySet();
                    System.out.println("\n\nRO::roImplementation(): Obtained Set of Nodes with Host " + setOfNodeswithHosts);
                    Node[] arrayOfNodeswithHosts = setOfNodeswithHosts.toArray(new Node[setOfNodeswithHosts.size()]);
                    for(Node N: arrayOfNodeswithHosts){
                        System.out.println ("\n\nRO::roImplementation(): Node is " + N);
                        Set<NodeConnector> setofNodeConn = nodeswithNodeConnectorHosts.get(N);
                        String NCid2;
                        for(NodeConnector NC : setofNodeConn)
                        {
                            NCid2 = NC.getNodeConnectorIdAsString();
                            if(NCid2.equals("OF|6")){
                                setofNodeConn.remove(NC);
                    ...
                    for(Node N : newTopology.keySet())
                    {
                       if(!setOfNodeswithHosts.contains(N))
                           newTopology.remove(N);
                    }
                    
                    System.out.println("\n\nRO::roImplementation(): New Topology Map of Nodes and Number of Hosts is " + newTopology);
                    Switchfailovercheck();
                    System.out.println("\n\nRO::roImplementation(): Exceuted SwitchFailoverCheck Function");
                }
 ...
 /*
    ...
    {
        Integer a = 0;
        JSONObject HostConfig = gethostTracker("admin","admin","http://192.168.56.21:8080/controller/nb/v2/hosttracker/default/hosts/active");
...
//            int count;
            ...
            System.out.println("\n\nRO::Switchfailovercheck(): NEW TOPOLOGY MAP OF HOSTS PER SWITCH IS " + newTopology);
            System.out.println("\n\nRO::Switchfailovercheck(): CURRENT TOPOLOGY MAP OF HOSTS PER SWITCH IS " + currentTopology);
            ...
            if(!newTopology.isEmpty()){
                for(Node N : arrayofNodesinCT)
                {
                    ...
                    if(p==true && q==true){
                        ...
                        if(c==d){
                            System.out.println("\n\nRO::Switchfailovercheck(): NUMBER OF HOSTS/SWITCH IS SAME ");}
                        else if( c - d > 0)
                        {   Timer2 = System.currentTimeMillis();
                           
                               
                                    System.out.println("\n\nRO::Switchfailovercheck(): CHANGE IN TOPOLOGY : NUMBER OF HOSTS PER SWITCH IS NOT SAME ");
                                    if(HostsinOtherController!=null)
                                    {
                                        neww = HostsinOtherController.size();
                                        System.out.println("\n\nRO::Switchfailovercheck(): VALUE OF OLD SIZE " + old + " VALUE OF NEW SIZE " + neww);
                                        if(old != neww)
                                        {
                                            fval = neww - old;
                                            System.out.println("\n\nRO::Switchfailovercheck(): NUMBER OF NEW HOSTS ADDED TO OTHER CONTROLLER IS " + fval);
                                            old = neww;
                                            
                                            // TO CHECK IF SWITCH SHOULD BE MOVED TO OTHER CONTROLLER
                                          //  if(failoverCheck.containsKey(N))
                                          //  { count = failoverCheck.get(N); }
                                          //  else { count =0; }
                                            count = count + 1;
                                           // failoverCheck.put(N,count);
                                            ...
                                            if(check >= 2)
                                            {
                                                System.out.println("\n\nRO::Switchfailovercheck(): SEND COMMAND TO PARENT TO MOVE SWITCH TO OTHER CONTROLLER");
                                                sendToMaster(1,N);
                                                
                                            }
                                            else
                                            {
                                                System.out.println("\n\nRO::Switchfailovercheck(): REG OPTIMIZATION THRESHOLD NOT REACHED");
                                            }
                                        }
                                    }
                        ...
                        System.out.println("\n\nRO::Switchfailovercheck(): SAME SWITCHES NOT PRESENT IN CURRENT AND NEW TOPOLOGY");
                    }
                }
            }
            else{
                System.out.println("\n\nRO::Switchfailovercheck(): NEW TOPOLOGY IS EMPTY ");
            }
        }
        ...
        for (Node everyNode : newTopology.keySet())
        {
            ...
            System.out.println("\n\nRO::Switchfailovercheck(): REACHED END OF SWITCHFAILOVER FUNCTION");
        }
    }
    
    public void sendToMaster(int type, Node N) {        // Type 1 : Move Switch to Other Controller
        try {
            System.out.println("\n\nRO::sendToMaster(): REGION OPTIMIZATION :  CONNECTING TO SOCKET OF MASTER ");
            Socket client = new Socket(MasterIP, MasterPort);
            OutputStream outToServer = client.getOutputStream();
            DataOutputStream out = new DataOutputStream(outToServer);
            String toSend;
            String idd = N.getNodeIDString();
            System.out.println("\n\nRO::sendToMaster(): Node ID is " + idd);
            if(type==1) {
                out.writeUTF(idd);
            }
            System.out.println("\n\nRO::sendToMaster(): REGION OPTIMIZATION: NODE ID SENT IS " + idd);
        }
